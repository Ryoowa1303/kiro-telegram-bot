...
This bot connects Telegram to **Kiro CLI running on your machine**. Anyone who
can message the bot can make Kiro **read/write files and run shell commands** on
...
3. **Understand `KIRO_TRUST_ALL_TOOLS=true`.** This runs tools without
   confirmation. Set it to `false` if you want Kiro to surface permission
...
- It does not transmit your code or secrets anywhere except to Telegram (your
  messages) and to Kiro CLI (which talks to its own backend).
